...
import { mockProducts, formatCurrency } from "@/data/platformMockData";
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1400px]">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-[#888] mt-1">{mockProducts.length} produtos cadastrados</p>
        </div>
        <Button
        ...
        {mockProducts.map((p, i) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${typeColors[p.type]}`}>
                {p.type}
              </span>
            </div>
            <h3 className="text-base font-semibold text-white mb-1">{p.name}</h3>
            <p className="text-lg font-bold text-white mb-1">{formatCurrency(p.price)}</p>
            <p className="text-xs text-[#888] mb-5">{p.totalSales} vendas realizadas</p>
